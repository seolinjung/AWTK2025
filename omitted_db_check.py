...
def compare_emails(folder_date, file1_stem, file2_stem):
    ...
    if only_in_first.empty:
        print(f"\n✅ 모든 email이 '{file1_stem}.xlsx'와 '{file2_stem}.xlsx' 양쪽에 존재합니다.\n")
    else:
        print("\n3. 📋 비교 결과:")
        print(f"👉 '{file1_stem}.xlsx'에만 있고, '{file2_stem}.xlsx'에는 없는 Email 목록:")
        print("-" * 60)
        print(only_in_first['Email'].to_string(index=False))
        print(f"\n총 {len(only_in_first)}개의 이메일이 '{file1_stem}.xlsx'에만 존재합니다.\n")
